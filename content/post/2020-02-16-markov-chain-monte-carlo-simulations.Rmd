---
title: Markov Chain Monte Carlo (MCMC) Simulations 
author: yjtek
date: '2020-02-16'
slug: markov-chain-monte-carlo-simulations
categories: []
tags: []
---
...
![](/post/2020-02-16-markov-chain-monte-carlo-simulations_files/2020-mcmc-Circle-inscribed-in-a-square (dots).jpg)
...
The Metropolis Algorithm is summarised below:
...
mu_frequentist_multiDim <- as.data.frame(x = metropolis_algo_multiDim(student_results, 71.2, proposed_sigma = 15, niter = niter)) %>%
  ...
  mutate(startingPoint = 'Frequentist')
mu_colleague_multiDim <- as.data.frame(x = metropolis_algo(student_results, 55, proposed_sigma = 5, niter = niter)) %>%
    mutate(status_mu = if_else(accepted_mu == proposed_mu, 'Accepted', 'Rejected'),
           status_sigma = if_else(accepted_sigma == proposed_sigma, 'Accepted', 'Rejected')) %>%
  mutate(startingPoint = 'Colleague')
mu_optimist_multiDim <- as.data.frame(x = metropolis_algo(student_results, 90, proposed_sigma = 50, niter = niter)) %>%
    mutate(status_mu = if_else(accepted_mu == proposed_mu, 'Accepted', 'Rejected'),
           status_sigma = if_else(accepted_sigma == proposed_sigma, 'Accepted', 'Rejected')) %>%
  mutate(startingPoint = 'Optimist')
```
